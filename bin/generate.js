...
function createComponentDir(name, type) {
  const typeDir = resolve(COMPONENT_DIR, `./${type}s`);
  mkdir(`${typeDir}/${name}`, (error) => {
    if (error && error.code === 'EEXIST') {
      console.log(`Component with name=${name} already exists. Please provide a different name`); // eslint-disable-line no-console
      process.exit(1);
    }
  });

  return resolve(typeDir, `./${name}`);
}

function createFile(parentDir, fileName, content) {
  writeFile(`${parentDir}/${fileName}`, content, (error) => {
    if (error) throw error;
    console.log(fileName, 'has been created!'); // eslint-disable-line no-console
  });
}
...
function add(name, type) {
  if (!isPascalCase(name)) {
    console.log('Please make sure that the Component name is PascalCase.'); // eslint-disable-line no-console
    process.exit(1);
  }
